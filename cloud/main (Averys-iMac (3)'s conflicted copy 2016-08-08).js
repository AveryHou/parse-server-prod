...
//(for collection view)
Parse.Cloud.define("getFoodStoreCollection", function(request, response) {
	var query = new Parse.Query("HBFoodStore");
	query.equalTo("online", true); 
  	query.find({
    	success: function(results) {
    		...
    		var rightStores = [];
	 		for (var i = 0; i < results.length; i++) { 
	       	 	...
	       	 	if (storeObj.get("displayAt") == "right") {
	       	 		rightStores.push(storeObj);
	       	 	} else {
	       	 		leftStores.push(storeObj);
	       	 	}
			}
	   	 	stores.push(rightStores);
	   	 	stores.push(leftStores);
	   	 	response.success(stores);
    	},
    	error: function(err) {
			console.error("food store lookup failed" + err.code + "," + err.message);
      	  	response.error(err);
    	}
  	});
});
...
Parse.Cloud.define("getSameLocationStore", function(request, response) {
	...
	if(request.params.cartMode != null && request.params.cartMode == "join") { //團購模式
		query.notContainedIn("onhold", ["breaking", "busy"]);  	
	}
	...
	query.find({
      ...
      error: function(err) {
			console.error("getSameLocationGroup lookup failed" + err.code + "," + err.message);
    	  	response.error(err);
		}
    });
});
			...
			query.find({
		    	success: function(itemsFound) {
		    		...
		    		for(var i=0 ; i<itemsFound.length ; i++) {
		     		...
		     		if (request.params.status == "onbid") {
		     			if (prop.mock_mode()) {  //是否為測試模式
		     				cartFound.set("status", "complete");
		     			} else {
		     				cartFound.set("status", "waitDispatch"); // https://hungrybee.atlassian.net/browse/SYS-1
		     			}
		     		} else {
						        	...
						        	if(request.params.rememberCreditCart == "YES") {
						 		    ...
						 		    currentUser.save(null, {
						        		success: function(userUpdated) {
						        			...
						        			body += "歐付寶交易序號: " + cartFound.get("allPayNo") + "<BR>";
...
Parse.Cloud.define("submitQRCode", function(request, response) {
	...
	query.get(request.params.cartId, {
	  	success: function(cartFound) {
	  		if (cartFound.get("status") == "waitDispatch") { // https://hungrybee.atlassian.net/browse/SYS-1
	...
	if(request.params.notify == "bee") { 
	    ...
	    query.get(request.params.cartId)
	    	.then(function(cartFound) {
	    		if(cartFound.get("status") == "waitDispatch" || cartFound.get("status") == "onbid") {
	...
	query.find()
		...
		.then(
			function(storeSlotsFound, availableSlotFound, storeBizDateFound){
				...
				var reasonsForEachStore = []; //此時段不可選的原因
				for (var i=0 ; i<availableSlotFound.length ; i++) {
					...
					for(var j=0 ; j<storeDataArray.length ; j++) { // loop each store
						...
						var slotIsAvailableForThisStore = [];
							...
							if (availableSlotFound[i].id == p.get("timeSlot").id) {
								...
								if (request.params.isToday == "Yes") {
									if (storeObj.get("reservationUnit") == "day") {
										slotIsAvailableForThisStore.push(storeObj.get("storeName") + "-需提前" + storeObj.get("reservation") + "天訂購");
									} else if (storeObj.get("reservationUnit") == "minute") {
										if (p.get("sinceMidnight") < currentTime + storeObj.get("reservation")) {
											slotIsAvailableForThisStore.push(storeObj.get("storeName") + "-需提前" + storeObj.get("reservation") + "分鐘訂購");
										} else {
											slotIsAvailableForThisStore.push("OK");
										}
									} else {
										if (!p.get("serviceOpen")) {
											slotIsAvailableForThisStore.push(storeObj.get("storeName") + "-休息時段");
										} else {
											slotIsAvailableForThisStore.push("OK");
										}
									}
									...
									if (!storeOpenOnDay) {
										slotIsAvailableForThisStore.push(storeObj.get("storeName") + "-公休");
									} else {
										if (!p.get("serviceOpen")) {
											slotIsAvailableForThisStore.push(storeObj.get("storeName") + "-休息時段");
										} else {
						...
						reasonsForEachStore.push(slotIsAvailableForThisStore);
					}
				}
				console.log("reasonsForEachStore:" + reasonsForEachStore);
				response.success(availableSlotFound);
			}, 
			function(error) {
				response.error(error);
			});
